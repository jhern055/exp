<?php $sessioMode=$this->session->userdata("sessionMode_shcp_file");?>
...
$("div.mShcp_file > div.cerUp > div#files > tr").each(function(i){
    ...
    buttons: {
        Si: function() {
			...
			$.ajax({
				    ...
				    url: url+"config/invoice/shcp_file/shcp_file_delete",
				    type: 'POST',
				    dataType: 'json',
				    data: {
				    	id:id
				    ...
				    success: function(response){
							...
							modal: true,
							    buttons: {
							        ...
							        if(response.status)
            						{window.location.href="<?php echo base_url().'config/invoice/shcp_file/'; ?>";}
