<?php $sessioMode=$this->session->userdata("sessionMode_purchase_order");?>
...
form.submit=function(mode,item){
	...
	// id_purchase_order :"<?php  echo (isset($id)?$id:'');?>",
    ...
    buttons: {
        Si: function() {
			...
			$.ajax({
				    ...
				    url: url+"admin/purchase/order/purchase_delete",
				    type: 'POST',
				    dataType: 'json',
				    data: {
				    	id:id
				    	...
				    	if(response.status){
							...
							modal: true,
							    buttons: {
            						...
            						window.location.href="<?php echo base_url().'admin/purchase/order/'; ?>";
...
if(MODE=="do_it"){
    ...
    $.ajax({
        ...
        async:true,
        dataType:"json",
        data:{details:details,MODE:MODE,edit:true}, 
        beforeSend:  function(response) {
        },
        success: function(response){
...
$(document).on("click","div.articleListContainer > div.area2 > div.data > div.itemArticle > div.editionActions > button.cancel",function() {
 ...
 obj.url="<?php echo base_url();?>";
	...
	$.ajax({
        ...
        async:true,
        dataType:"json",
        data:{id: obj.id,source_module:"admin/purchase/order/" }, 
        beforeSend:  function(response) {
        },
        success: function(response){
	...
	function chargeCancel (details) {
	    ...
	    $.ajax({
	        ...
	        async:true,
	        dataType:"json",
	        data:{details:details,MODE:"do_it",edit:true}, 
	        beforeSend:  function(response) {
	        },
	        success: function(response){
