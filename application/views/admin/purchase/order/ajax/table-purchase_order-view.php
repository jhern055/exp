<div id="dataTables-example_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
        ...
        <div class="col-sm-12">
            ...
            <?php foreach($records_array as $rows): ?> 
                ...
                <tr class="gradeA odd" role="row" data-row_id="<?php echo encode_id($rows["id"]); ?>" data-provider_email="<?php echo (!empty($rows["provider_email"])?$rows["provider_email"]:""); ?>">
                    <td class="center"><?php echo (!empty($rows["folio"])? anchor(base_url()."admin/purchase/order/orderView/".$rows["id"],$rows["folio"]) :"&nbsp;") ?></td>
                    <td class="center"><?php echo (!empty($rows["provider_name"])?anchor(base_url()."provider/providerView/".$rows["provider_id"],$rows["provider_id"]."-".$rows["provider_name"]):"&nbsp;") ?></td>
                    ...
                    <td class="center"><?php echo anchor(base_url()."admin/payment/admin/?source_module=".encode_id("admin/purchase/payment/")."&id=".encode_id($rows["id"])."&module=".encode_id("purchase"),"<span class='payment_search'></span>",""); ?></td>
