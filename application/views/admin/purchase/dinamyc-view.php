<?php $sessioMode=$this->session->userdata("sessionMode_purchase");?>
...
form.submit=function(mode,item){
	...
	// id_purchase :"<?php  echo (isset($id)?$id:'');?>",
    ...
    buttons: {
        Si: function() {
			...
			$.ajax({
				    ...
				    url: url+"admin/purchase/purchase_delete",
				    type: 'POST',
				    dataType: 'json',
				    data: {
				    	id:id
				    	...
				    	if(response.status){
							...
							modal: true,
							    buttons: {
            						...
            						window.location.href="<?php echo base_url().'admin/purchase/'; ?>";
...
$(document).on("click","div.articleListContainer > div.area2 > div.data > div.itemArticle > div.editionActions > button.cancel",function() {
 ...
 obj.url="<?php echo base_url();?>";
	...
	$.ajax({
        ...
        async:true,
        dataType:"json",
        data:{id: obj.id,source_module:"admin/purchase/" }, 
        beforeSend:  function(response) {
        },
        success: function(response){
