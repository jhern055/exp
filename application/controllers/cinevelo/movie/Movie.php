...
// ------------------------
	...
	$module=(!empty($module)?$module:"cinevelo/movie/");
	$data["module"]=$module;	
			...
			// mover y insertar en el registro
				...
				$this->load->helper("file");
					...
					foreach ($tmp_move as $tmp_mv_name) {
						...
						$response=move_file("movie_image",$tmp_mv_name["basename"],$last_id);
...
// <movie> 
	...
	$this->pagination->initialize($this->config_pagination("cinevelo/movie/movie_ajax",$this->movie_model->get_movie_amount($query_search),$page_amount) );
// <OWN>
...
// </OWN>
	...
	$data["module_data"]["module_data_method_do_it"]="cinevelo/movie/movieView/";

    $data["modules_quick"]=$this->load->get_back_access($module);

	$html=$this->load->view("cinevelo/movie/ajax/table-movie-view",$data,true);
	$data["html"]=$html;
	...
	if(!empty($_POST["ajax"]))
	return print_r(json_encode(array("status"=>1,"msg"=>"HtmlConExito","html"=>$this->load->view("cinevelo/movie/movie_view",$data,true) ))) ;
	else
	return $this->load->template("cinevelo/movie/movie_view",$data);
	...
	$array['data']["module_data_method_do_it"]="cinevelo/movie/movieView/";
