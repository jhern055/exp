...
class PurchaseOrder extends CI_Controller {
	...
	$this->load->model("admin/purchase/order/purchase_order_model");
    ...
    $uri_string=(!empty($uri_string["uri_string"])? $uri_string["uri_string"]:$this->uri->uri_string );
		...
		$this->uri_purchase_order="admin/purchase/order/";
		// </purchase_order>
	}
	...
	// orderView
		...
		if($this->purchase_order_model->record_same_purchase_order($data,$id) )
		return array("status"=>0,"msg"=>"Ya existe un registro identico ","data"=>false);

		$data=array_merge($data_depend,$data);

		if($id)
		$last_id=$this->purchase_order_model->update_purchase_order($data,$id);
		else
		$last_id=$this->purchase_order_model->insert_purchase_order($data);

		// processar details
		$response_detail=$this->processDetail("purchase_order",$last_id);
...
// <purchase_order> 
	...
	// carga ajax
	public function purchase_order_ajax() {

	$module=$this->uri_purchase_order;

	$http_params=array_merge($_GET,$_POST);
	$http_params          =array(
	"input_search_purchase_order" =>(!empty($http_params["input_search_purchase_order"]) ? strip_tags( $this->security->xss_clean( $http_params["input_search_purchase_order"] ) ) :""),
	"show_amount_purchase_order"  =>(!empty($http_params["show_amount_purchase_order"]) ? strip_tags( $this->security->xss_clean( $http_params["show_amount_purchase_order"])  ) :10),
	);
	extract($http_params);

	$page_amount=$show_amount_purchase_order;

	$query_search=array(
		"\$this->db->like('folio', \"".$input_search_purchase_order."\");",
		);

	$this->pagination->initialize($this->config_pagination("admin/purchase/order/purchase_order_ajax",$this->purchase_order_model->get_purchase_order_amount($query_search),$page_amount) );

	$data=array(
		"module"=>$module,
		"sys"=>$this->sys,
		"input_search_purchase_order"=>$input_search_purchase_order,
		"show_amount"=>$show_amount_purchase_order,
		"records_array"=>$this->purchase_order_model->get_purchase_order($page_amount, $this->page,$query_search),
		"pagination"=>$this->pagination->create_links(),
		"module_data"=>$this->purchase_order_model->m_name($module),
		);
	$data["module_data"]["module_data_method_do_it"]="admin/purchase/order/orderView/";

    $data["modules_quick"]=$this->load->get_back_access($module);

	$html=$this->load->view("admin/purchase/order/ajax/table-purchase_order-view",$data,true);
	$data["html"]=$html;

	// $this->session->set_userdata('record_start_row_purchase_order',$this->page);
	...
	$data=$this->purchase_order_ajax();

	$this->session->set_userdata("idTemp");
	$this->session->set_userdata('input_search_purchase_order');
	$this->session->set_userdata("sessionMode_purchase_order");

	// if(!empty($this->page) and !empty($data["records_array"]))
	// $this->session->set_userdata('record_start_row_purchase_order',$this->page);

	if(!empty($_POST["ajax"]))
	return print_r(json_encode(array("status"=>1,"msg"=>"HtmlConExito","html"=>$this->load->view("admin/purchase/order/purchase_order_view",$data,true) ))) ;
	else
	return $this->load->template("admin/purchase/order/purchase_order_view",$data);
	...
	$return=$this->do_it($id,"orderView");
	...
	// traer el registro 
	$array['data']=$this->purchase_order_model->get_purchase_order_id($id);
	...
	// <F5>
	$sessionMode=$this->session->userdata("sessionMode_purchase_order");
	if(!empty($_POST)):
		$this->session->set_userdata("sessionMode_purchase_order",$array['data']['MODE']);
	endif;
	$array['id']=$id;
	// </F5>

	// nombre del modulo
	$array['data']["module_data"]=$this->purchase_order_model->m_name($module);	
	$array['data']["module_data_method_do_it"]="admin/purchase/order/orderView/";
	...
	if($this->purchase_order_model->purchase_order_delete_it($id))
