...
class MenuPrimary extends CI_Controller {
    ...
    // menuPrimary
	public 	$uri_menuPrimary;
    // ..

	public function __construct() {
	parent::__construct();		

	$this->load->model("web/menus/primary/menuPrimary_model");
	...
	// Sucursal <menuPrimary>
	$this->uri_menuPrimary="web/menus/primary/";
	// ... </menuPrimary>
	...
	// menuPrimaryView
		...
		if($this->menuPrimary_model->record_same_menuPrimary($data,$id) )
		return array("status"=>0,"msg"=>"Ya existe un registro identico ","data"=>false);

		$data=array_merge($data_depend,$data);

				if($id)
				$last_id=$this->menuPrimary_model->update_menuPrimary($data,$id);
				else
				$last_id=$this->menuPrimary_model->insert_menuPrimary($data);
	...
	$module=(!empty($module)?$module:"web/menus/primary/");
	...
	// <menuPrimary>
	// carga ajax
	public function menuPrimary_ajax() {

	$module=$this->uri_menuPrimary;

	$http_params=array_merge($_GET,$_POST);
	$http_params          =array(
	"input_search_menuPrimary" =>(!empty($http_params["input_search_menuPrimary"]) ? strip_tags( $this->security->xss_clean( $http_params["input_search_menuPrimary"] ) ) :""),
	"show_amount_menuPrimary"  =>(!empty($http_params["show_amount_menuPrimary"]) ? strip_tags( $this->security->xss_clean( $http_params["show_amount_menuPrimary"])  ) :10),
	);
	extract($http_params);

	$page_amount=$show_amount_menuPrimary;

	$query_search=array(
		"\$this->db->like('name', \"".$input_search_menuPrimary."\");",
		);

	$this->pagination->initialize($this->config_pagination("web/menus/primary/menuPrimary_ajax",$this->menuPrimary_model->get_menuPrimary_amount($query_search),$page_amount) );

	$data=array(
		"module"=>$module,
		"sys"=>$this->sys,
		"input_search_menuPrimary"=>$input_search_menuPrimary,
		"show_amount"=>$show_amount_menuPrimary,
		"records_array"=>$this->menuPrimary_model->get_menuPrimary($page_amount, $this->page,$query_search),
		"pagination"=>$this->pagination->create_links(),
		"module_data"=>$this->menuPrimary_model->m_name($module),
		);
	$data["module_data"]["module_data_method_do_it"]="web/menus/primary/menuPrimaryView/";

    $data["modules_quick"]=$this->load->get_back_access($module);

	$html=$this->load->view("web/menus/primary/ajax/table-menuPrimary-view",$data,true);
	$data["html"]=$html;

	// $this->session->set_userdata('record_start_row_menuPrimary',$this->page);
	...
	$data=$this->menuPrimary_ajax();
	// pr($data);
	$this->session->set_userdata("idTemp");
	$this->session->set_userdata('input_search_menuPrimary');
	$this->session->set_userdata("sessionMode_menuPrimary");

	// if(!empty($this->page) and !empty($data["records_array"]))
	// $this->session->set_userdata('record_start_row_menuPrimary',$this->page);

	if(!empty($_POST["ajax"]))
	return print_r(json_encode(array("status"=>1,"msg"=>"HtmlConExito","html"=>$this->load->view('web/menus/primary/menuPrimary_view',$data,true) ))) ;
	else
	return $this->load->template('web/menus/primary/menuPrimary_view',$data);

	}
	// ver para registro
	public function menuPrimaryView($id=null) {
	...
	$return=$this->do_it($id,"menuPrimaryView");
	...
	// traer el registro 
	$array['data']=$this->menuPrimary_model->get_menuPrimary_id($id);
	...
	// <F5>
	$sessionMode=$this->session->userdata("sessionMode_menuPrimary");
	if(!empty($_POST)):
		$this->session->set_userdata("sessionMode_menuPrimary",$array['data']['MODE']);
	endif;
	$array['id']=$id;
	// </F5>
	// nombre del modulo
	$array['data']["module_data"]=$this->menuPrimary_model->m_name($module);	
	$array['data']["module_data_method_do_it"]="web/menus/primary/menuPrimaryView/";
	...
	public function menuPrimary_delete() {
	...
	if($this->menuPrimary_model->menuPrimary_delete_it($id))
