...
local call_if_not_empty = function(fun, state_x, ...)
    ...
    local params = {...}
    if #params == 1 then
        -- XXX added state_x to receive underscore's style `callback(v, k)`
        return state_x, fun(params[1], state_x)
    else
        -- XXX changed args order to receive underscore's style `callback(v, k)`
        return state_x, fun(params[2], params[1])
    end
end
...
local filter1_gen = function(fun, gen_x, param_x, state_x, a)
    while true do
        -- XXX added state_x to receive underscore's style `callback(v, k)`
        if state_x == nil or fun(a, state_x) then break; end
...
filterm_gen = function(fun, gen_x, param_x, state_x, ...)
    ...
    local args = {...} -- XXX inverted args to receive underscore's style `callback(v, k)`
    if fun(args[2], args[1]) then
        return state_x, ...
    end
